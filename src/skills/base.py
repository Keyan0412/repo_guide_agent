...
class BaseSkill(ABC):
    ...
    def run(self, skill_input: SkillInput, context: AgentContext, logger: AgentLogger | None = None) -> SkillOutput:
        logger = logger or AgentLogger(tool_logs=context.tool_logs, workflow_history=context.workflow_history)
        logger.emit(f"[skill:start] {self.name}")
        if not self.llm_client.enabled:
            raise AgentError(f"skill:{self.name}", "LLM client is unavailable. Check OPENAI_API_KEY / OPENAI_BASE_URL / OPENAI_MODEL.")
        llm_output = self._run_with_llm(skill_input, context, logger)
        if llm_output is None:
            raise AgentError(f"skill:{self.name}", "LLM tool-agent failed to return valid structured output.")
        logger.emit(f"[skill:done] {self.name} via llm")
        return llm_output
    ...
    def _run_structured_llm(
        self,
        skill_input: SkillInput, context: AgentContext, logger: AgentLogger,
        *,
