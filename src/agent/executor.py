...
class Executor:
    ...
    def execute_plan(
        ...
        logger.emit(f"[executor:start] repo={parsed_query.repo_path}")
        ...
        while current_node_id:
            ...
            logger.log_workflow_node(node.node_id, node.node_type, node.name, "start")
            output = self._execute_node(node, parsed_query, context, logger)
            logger.log_workflow_node(node.node_id, node.node_type, node.name, "done")
            if output is not None:
                outputs.append(output)
            current_node_id = self._next_node_id(plan, node.node_id, context)
        logger.emit("[executor:done] all skills completed")
        context.tool_logs = logger.tool_logs
        context.workflow_history = logger.workflow_history
        return context, outputs

    def _build_initial_state(self, parsed_query: ParsedQuery, plan: ExecutionPlan) -> WorkflowState:
        return WorkflowState.from_query(parsed_query, plan)
    ...
    def _execute_node(
        ...
        logger.emit(f"[executor] running step={node.name} args={node.args}")
        merged_input = self._build_skill_input(parsed_query, node.args, context.workflow_state)
        output = self.skills[node.name].run(merged_input, context, logger)
    ...
    def _apply_skill_output(self, node: WorkflowNode, output: SkillOutput, context: AgentContext) -> None:
        completed_nodes = list(context.workflow_state.completed_nodes)
        completed_nodes.append(node.node_id)
        completed_skills = list(context.workflow_state.completed_skills)
        completed_skills.append(output.skill_name)
        outputs_by_skill = dict(context.workflow_state.outputs_by_skill)
        outputs_by_skill[node.node_id] = output.data

        evidence_pool = list(context.workflow_state.evidence_pool)
        evidence_pool.extend(item for item in output.evidence if item not in evidence_pool)

        findings = list(context.workflow_state.findings)
        ...
        open_questions = list(context.workflow_state.open_questions)
        ...
        if output.skill_name == "investigate_question":
            workflow_updates["investigation_round"] = context.workflow_state.investigation_round + 1
    ...
    def _edge_is_open(self, condition: str, context: AgentContext) -> bool:
        ...
        if condition == "needs_followup":
            return (
                not context.workflow_state.answer_ready
                and context.workflow_state.investigation_round < context.workflow_state.max_investigation_rounds
            )
        return False

    def _build_skill_input(self, parsed_query: ParsedQuery, call_args: dict, workflow_state: WorkflowState) -> SkillInput:
        base = SkillInput(
            repo_path=parsed_query.repo_path,
            ...
            investigation_focus=list(workflow_state.open_questions),
